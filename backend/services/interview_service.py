...
def delete_interview(db: Session, identifier: str) -> bool:
    """
    Deletes an interview record cleanly by interview id, candidateId, applicationId, email, or name.
    Reverts candidate & application status in PostgreSQL to Shortlisted when interviews are deleted.
    """
    if not identifier:
        return False
    ...
    matches = db.query(InterviewModel).filter(
        or_(
            ...
            func.lower(InterviewModel.candidateName).ilike(f"%{raw_id.lower()}%"),
            func.lower(InterviewModel.candidateEmail) == raw_id.lower()
        )
    ).all()
        ...
        if cand:
            matches = db.query(InterviewModel).filter(
                or_(
                    ...
                    func.lower(InterviewModel.candidateEmail) == (cand.email or "").lower()
                )
            ).all()
    ...
    if not matches:
        from models.candidate import CandidateModel
        from models.application import ApplicationModel
        # Search candidates by name or email substring
        cands = db.query(CandidateModel).filter(
            or_(
                func.lower(CandidateModel.firstName).ilike(f"%{raw_id.lower()}%"),
                func.lower(CandidateModel.lastName).ilike(f"%{raw_id.lower()}%"),
                func.lower(CandidateModel.email) == raw_id.lower()
            )
        ).all()
        for cand in cands:
            cand_matches = db.query(InterviewModel).filter(
                or_(
                    InterviewModel.candidateId.in_([cand.id, cand.candidateId]),
                    func.lower(InterviewModel.candidateEmail) == (cand.email or "").lower()
                )
            ...
            if cand_matches:
                matches.extend(cand_matches)

    # Revert candidate & application status if no interview row found but candidate is in Interviewing stage
    if not matches:
        cand = candidate_service.get_candidate_by_id_or_candidate_id(db, raw_id)
        if cand and ("interview" in str(cand.status).lower()):
            now = datetime.datetime.now(datetime.timezone.utc).isoformat()
            cand.status = "Shortlisted"
            cand.updatedAt = now
            apps = db.query(ApplicationModel).filter(ApplicationModel.candidateId.in_([cand.id, cand.candidateId])).all()
            for app in apps:
    ...
    # Revert candidate & application status to Shortlisted
    now = datetime.datetime.now(datetime.timezone.utc).isoformat()
    if app_id:
